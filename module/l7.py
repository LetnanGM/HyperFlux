            ...
            async def send_request(session, url, retries=3):
                ...
                try:
                    async with session.get(url, headers=headers) as response:
                        print(Fore.LIGHTGREEN_EX, f"[HyperFlux] Url : {url} GET Request : {get_request} Status : {response.status}")
                except aiohttp.ClientConnectorError:
                    print(Fore.GREEN, f"[-] Server has down by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
                except aiohttp.ServerDisconnectedError:
                    print(Fore.GREEN, f"[-] Server has disconnected by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
                except asyncio.TimeoutError:
                    print(Fore.GREEN, f"[-] Server has TIMEOUT by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
...
╚══> """))
            ...
            print(Fore.WHITE, "[+] Loading HyperFlux...")
            ...
            async def send_request(session, url, retries=3):
                ...
                try:
                    async with session.post(url, headers=headers) as response:
                        print(Fore.LIGHTGREEN_EX, f"[HyperFlux] Url : {url} POST Request : {post_request} Status : {response.status}")
                except aiohttp.ClientConnectorError:
                    print(Fore.GREEN, f"[-] Server has down by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)  
            ...
            async def send_request(session, url, retries=3):
                ...
                try:
                    async with session.head(url, headers=headers) as response:
                        print(Fore.LIGHTGREEN_EX, f"[HyperFlux] Url : {url} HEAD Request : {head_request} Status : {response.status}")
                except aiohttp.ClientConnectorError:
                    print(Fore.GREEN, f"[-] Server has down by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)  
...
╚══> """))
            ...
            async def send_request(client, url, retries=3):
                ...
                try:
                    ...
                    print(Fore.LIGHTGREEN_EX, f"[HyperFlux] Url : {url} HTTP2 Request : {get_request} Status : {response.status_code}")
...
╚══> """))
            ...
            async def send_request(session, url, proxy, retries=3):
                ...
                try:
                    async with session.get(url, headers=headers) as response:
                        print(Fore.LIGHTGREEN_EX, f"[HyperFlux] Url : {url} PROXY Request : {get_request} Status : {response.status}")
                except aiohttp.ClientConnectorError:
                    print(Fore.GREEN, f"[-] Server has down by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                    ...
                    print(Fore.GREEN, f"[-] Server disconnected by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, proxy, retries - 1)
                except asyncio.TimeoutError:
                    print(Fore.GREEN, f"[-] Server TIMEOUT by HyperFlux !!")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, proxy, retries - 1)
